...
class UCB(College):
    ...
    def verify_add_request(request):
        ...
        if term not in UCB.term_names:
            return errors("Invalid term", "add")
        if not UCB.PATTERN_CRN.match(crn):
    ...
    def monitor_from_add_request(request):
        ...
        class_instance = UCB.Class(crn, term)
    ...
    class Class(College.Class):
        ...
        vsub = re.compile(r"(dept=)([A-Z]+)")
        vcrse = re.compile(r"(course=)(\d+)")
        availability = r"<td class=\"default3\">(-?\d*)</td>"

        def __init__(self, crn, term):
            super().__init__()
            self.url = UCB.Class.base_url + ".p_course_popup?vterm={}&vcrn={}".format(term, crn)
            html = urlopen(self.url).read().decode()
            self.vcrn = crn
            self.vterm = term
            self.vsub = UCB.Class.vsub.search(html).group(2)
            self.vcrse = UCB.Class.vcrse.search(html).group(2)
            ...
            self.info_url = UCB.Class.base_url + ".p_course_popup?vsub={}&vcrse={}&vterm={}&vcrn={}".format(self.vsub,
                                                                                                            self.vcrse,
                                                                                                            self.vterm,
                                                                                                            self.vcrn)
            self.update_status()
        ...
        def update_status(self):
            html = urlopen(self.url).read().decode()
            a = re.findall(UCB.Class.availability, html)
